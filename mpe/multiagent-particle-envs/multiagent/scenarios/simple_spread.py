...
class Scenario(BaseScenario):
    ...
    def reward(self, agent, world):
        ...
        rew = 0
        rew_agents = [0 for i in range(len(world.agents))]
        # TODO please check minimum value of rewards
        for l in world.landmarks:
            dists = [np.sqrt(np.sum(np.square(a.state.p_pos - l.state.p_pos))) for a in world.agents]
            # rew -= min(dists)
            rew += min(1. / min(dists), 10.) # modified by ling
            # index = np.argmin(dists)[0]  # modified by CCA
            # rew_agents[index] += min(1. / min(dists), 10.) # modified by CCA
