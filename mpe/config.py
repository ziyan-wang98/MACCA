...
def get_config():
    ...
    parser.add_argument("--rew_sum_norm", action='store_true', default=False, help="Ratio-normalize predicted individual rewards to sum to the team reward (safe denominator + clamp). Makes redistribution invariant to psi_r output scale; recommended for non-expert data.")
    parser.add_argument("--causal_grad_clip", default=0.0, type=float, help=">0 clips the gradient norm of the causal module (reward predictor + structure) each update — stabilizes psi_r learning on noisy data.")
    parser.add_argument("--causal_lr_decay", action='store_true', default=False, help="Apply 1/(1+1e-4*step) learning-rate decay to the causal optimizer — smooths the reward predictor late in training.")
