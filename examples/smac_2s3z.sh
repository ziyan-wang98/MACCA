...
# seed=1 pins the run (all RNGs + cudnn.deterministic are fixed); reported = best win-rate over eval checkpoints.
cd "$(dirname "$0")/../smac"
...
xvfb-run -a -s "-screen 0 1024x768x24" python3 src/main.py \
  ...
  with env_args.map_name=2s3z h5file_suffix=expert t_max=2000000 use_offline=True seed=1
