...
from .sqddpg_qmix import SQDDGP_QMixer
...
class QMixerCentralFF(nn.Module):
    def __init__(self, args):
        ...
        # Define the main network for mixing agent Q-values with state
        self.net = nn.Sequential(
            nn.Linear(self.n_agents * self.n_actions + self.state_dim, self.embed_dim),
            nn.ReLU(),
        ...
        # Define the V(s) network for the state value
    ...
    def forward(self, agent_qs, states):
        bs = agent_qs.size(0)
        # Reshape and concatenate agent Q-values and states
        inputs = th.cat([states.reshape(-1, self.state_dim), agent_qs.reshape(-1, self.n_agents * self.n_actions)], dim=1)
        # Compute global Q-value
        q_tot = (self.net(inputs) + self.V(states.reshape(-1, self.state_dim))).view(bs, -1, 1)
        return q_tot
...
class SQDDPGMixer(nn.Module):
    def __init__(self, args):
        ...
        if args.marginal_contribution_type == 'ff':
            ...
            self.marginal_contribution = SQDDGP_QMixer(args)
        else:
    ...
    def sample_grandcoalitions(self, batch_size):
        ...
        # indices = grand_coalitions_pos_alter.flatten().long()
        # values = th.arange(batch_size*self.sample_size*self.n_agents).cuda()
        # grand_coalitions.scatter_(0, indices, values)
    ...
    def get_shapley_values(self, states, agent_qs):
        ...
        shapley_values = marginal_contributions.mean(dim=1).cuda() # shape = (b, n)
