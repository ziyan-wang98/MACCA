...
def build_td_lambda_targets_macca(rewards, terminated, mask, target_qs, n_agents, gamma, td_lambda):
    # Initialise last lambda-return for not terminated episodes
    ret = target_qs.clone()  # Start with the same shape as target_qs
    ret[:, -1] = target_qs[:, -1] * (1 - terminated.sum(dim=2))  # Adjust for termination

    terminated_expanded = terminated.unsqueeze(-1).expand(-1, -1, n_agents)
    mask_expanded = mask.unsqueeze(-1).expand(-1, -1, n_agents)

    # Backwards recursive update of the "forward view"
    for t in range(ret.shape[1] - 2, -1, -1):
        for agent_id in range(n_agents):
            ret[:, t, agent_id] = td_lambda * gamma * ret[:, t + 1, agent_id] + mask_expanded[:, t, agent_id] \
                                * (rewards[:, t, agent_id] + (1 - td_lambda) * gamma * target_qs[:, t + 1, agent_id] * (1 - terminated_expanded[:, t, agent_id]))
