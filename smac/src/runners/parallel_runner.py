...
class ParallelRunner:
    ...
    def _log(self, returns, stats, prefix, env_stats=None):
        ...
        if env_stats is not None:
            # If env_states have win_rates, log them
            if all('win_rate' in stat for stat in env_stats):
                win_rates = [stat.get('win_rate', 0) for stat in env_stats]
                if win_rates:
                    self.logger.log_stat(prefix + "win_rate_mean", np.mean(win_rates), t_env)
                    self.logger.log_stat(prefix + "win_rate_std", np.std(win_rates), t_env)
