...
import torch as th
from utils.rl_utils import build_td_lambda_targets, build_target_q,build_td_lambda_targets_macca
...
class MACCA_ICQLearner(ICQLearner):
    def __init__(self, mac, scheme, logger, args):
        super(MACCA_ICQLearner, self).__init__(mac, scheme, logger, args)
        self.args = args
    ...
    def train(self, batch: EpisodeBatch, t_env: int, log):
        ...
        log = self.train_critic(batch, t_env=t_env)
        ...
        coma_loss = - ((len(advantages) * advantages.detach() * log_pi_taken) * mask_td).sum() / mask.sum()
